...
Automated weekly playlist generation driven by your own listening history, built on top of [spotify-mcp-server](https://github.com/marcelmarais/spotify-mcp-server) by Marcel Marais.
...
Built on [spotify-mcp-server](https://github.com/marcelmarais/spotify-mcp-server) — an MCP server that gives AI assistants (Claude, Cursor) Spotify control.

<details>
<summary>Available MCP tools</summary>

**Read**: searchSpotify, getNowPlaying, getMyPlaylists, getPlaylistTracks, getRecentlyPlayed, getUsersSavedTracks, getQueue, getAvailableDevices

**Playback**: playMusic, pausePlayback, resumePlayback, skipToNext, skipToPrevious, addToQueue, setVolume, adjustVolume

**Playlists**: createPlaylist, addTracksToPlaylist
...
| `npm run auth` | OAuth flow (terminal) |
...
├── read.ts               # MCP read operation tools
├── play.ts               # MCP playback/playlist tools
├── albums.ts             # MCP album tools
├── auth.ts               # OAuth flow
...
├── cli/
│   ├── fill.ts           # CLI wrapper for playlist filler
│   ├── recalculate.ts    # CLI wrapper for priority calculator
│   ├── find-artist.ts    # CLI wrapper for artist lookup
│   ├── list-artists.ts   # CLI wrapper for listing artists
│   └── clear.ts          # CLI wrapper for playlist clearer
└── web/
    ├── server.ts         # Web dashboard server (Express + WebSocket)
