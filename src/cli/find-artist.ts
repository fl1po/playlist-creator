...
const service = new ArtistLookupService(client, {
  onResult: (r) => {
    ...
    console.log(`  ${r.name}`);
    console.log(`    Priority:   ${priorityLabel} (score: ${r.data.score})${posStr}`);
    console.log(`    Popularity: ${r.popularity ?? "?"}/100`);
    console.log(`    Followers:  ${r.followers?.toLocaleString() ?? "?"}`);
    console.log(`    Genres:     ${r.genres.length ? r.genres.join(", ") : "—"}`);
    console.log(`    AW tracks:  ${r.data.allWeekly}  |  BoAW tracks: ${r.data.bestOfAllWeekly}`);
    console.log(`    Recency:    AW +${r.data.recencyBonusAW}  |  BoAW +${r.data.recencyBonusBoAW}`);
    console.log(`    Spotify:    https://open.spotify.com/artist/${r.data.spotifyId || "?"}`);
    console.log();
  },
